...
public class PopDirectoryTest {
  ...
  public void testNonEmptyStack() {
    ...
    assertEquals(fileTree.getCurrent().getName(), folder2.getName());
    popd.execute(new String[] {"popd"}, fileTree);
    assertEquals(fileTree.getCurrent().getName(), folder1.getName());
    popd.execute(new String[] {"popd"}, fileTree);
    assertEquals(fileTree.getCurrent().getName(), root.getName());
  }
